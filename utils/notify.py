...
def notify_swap(swap, best_block_height, feedback):
    ...
    res = requests.post(feedback, data=data, headers={
                        'Content-Type': 'text/plain', 'signature': sign}, timeout=5)
    if res.status_code != 200:
        raise DepositNotifyException(
            'notify deposit exception, %s, %s' % (res.status_code, res.text))
